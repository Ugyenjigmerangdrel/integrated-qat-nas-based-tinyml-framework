...
model = build_model(input_shape, num_classes, int8bo_top_cfg)

initial_lr = 5e-4
opt = keras.optimizers.Adam(learning_rate=initial_lr)

train_loss, train_acc, val_loss, val_acc, test_loss, test_acc = train_qat_model(model, opt, X_train, y_train, X_val, y_val, X_test, y_test, "./models/int8bo/best_int8bo_qat_dscnn.weights.h5", "./models/best_int8bo_qat_dscnn.keras")
